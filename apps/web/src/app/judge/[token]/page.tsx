  ...
  return (
    <main className="min-h-screen bg-[#f4f6fa] flex flex-col">
        ...
        <div className="max-w-5xl mx-auto px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-slate-900/10 border border-slate-300 flex items-center justify-center text-3xl text-slate-700 font-bold">
                {judge?.name?.[0]}
              </div>
              <div>
                <h1 className="text-2xl font-bold text-slate-900">{judge?.name}</h1>
                <p className="text-base text-slate-600">{judge?.judgeType} Judge · {judge?.organisation}</p>
              </div>
            </div>
            {lastUpdated && (
      ...
      <div className={`w-full max-w-5xl mx-auto px-6 py-8 ${
        ...
        {activeScorecard && (
          <div className="mb-6 rounded-xl border border-slate-300 bg-white p-6">
              ...
              {!['SUBMITTED', 'RESUBMITTED', 'LOCKED'].includes(activeScorecard.status) ? (
                <div className="flex gap-2">
                  <button onClick={() => saveOrSubmit(false)} disabled={saving}
                    className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-900 text-base rounded-lg disabled:opacity-50">
                    {saving ? 'Saving...' : 'Save draft'}
                  </button>
                  <button onClick={() => saveOrSubmit(true)} disabled={saving}
                    className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white text-base font-medium rounded-lg disabled:opacity-50">
